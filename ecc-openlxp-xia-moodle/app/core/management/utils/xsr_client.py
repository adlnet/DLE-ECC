...
def filter_moodle_data(source_data_dict, xsr_api_end):
...
def custom_moodle_fields(source_data_dict, xsr_obj):
    ...
    # filter out only ECC approved courses
    source_ecc_approved_dict = \
        filter_moodle_data(source_data_dict, xsr_api_end)
    ...
    for source_course in source_ecc_approved_dict:
        ...
        try:
            ...
            contact_str = list_to_string(contacts)
        ...
        try:
            ...
            enrollment_str = list_to_string(source_course['enrollmentmethods'])
...
def find_html(data_dict):
    ...
    for element in data_flattened.keys():
        if data_flattened[element] and (bool(
            BeautifulSoup(str(data_flattened[element]),
                          "html.parser").find())):
